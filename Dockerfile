...
ENV CGO_ENABLED=0
RUN make bin/compose-controller bin/compose-controller.test e2e-binary bin/installer bin/updater bin/api-server bin/api-server.test
...
# compose-updater (retrieved with --target=compose-updater)
FROM runbase AS compose-updater
ENTRYPOINT ["/updater"]
COPY --from=build /go/src/github.com/docker/compose-on-kubernetes/bin/updater /updater
